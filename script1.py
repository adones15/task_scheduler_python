...
def func_s1(): #Função a ser agendada no arquivo principal (task_scheduler.py)
    try:
        ...
        df.to_excel("Script1.xlsx")

        #log caso a função seja executada SEM erros.
        with open(fr"G:\Meu Drive\Adones\Projetos_Python\task_scheduler\logs\log_sucesso_func_s1_data_{dtn[0:4]}{dtn[5:7]}{dtn[9:11]}_hora_{dtn[11:13]}{dtn[14:16]}{dtn[17:19]}.txt", "w+") as arquivo:
            arquivo.write("Executado com sucesso!!!")
    
    except:
        #log caso a função seja executada COM erros.
        dtn = str(datetime.datetime.now())
        with open(fr"G:\Meu Drive\Adones\Projetos_Python\task_scheduler\logs\log_erro_func_s1_data_{dtn[0:4]}{dtn[5:7]}{dtn[9:11]}_hora_{dtn[11:13]}{dtn[14:16]}{dtn[17:19]}.txt", "w+") as arquivo:
